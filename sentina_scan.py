...
def fun():
        url = entry.get()
        progressbar_1 = customtkinter.CTkProgressBar(main_frame,width=500,height=30)
        progressbar_1.place(x=200,y=400)
        progressbar_1.configure(mode="indeterminate")
        progressbar_1.start()
        scan_label.configure(text="Scanning....\"{}\"".format(url))
...
def main(url):
        def progress(texts,value=None):
            if value == None:
                progress_label.configure(text=texts)
            else:
                progress_label.configure(text=texts.format(value))
        ...
        def vul_remed_info(v1,v2,v3):
            ...
            res = f"Vulnerability Threat Level:\n{vul_info(v2)+str(tool_resp[v1][0])}"
            res2 = f"Vulnerability Definition:\n{str(tools_fix[v3-1][1])}"
            res3 = f"Vulnerability Remediation:\n{str(tools_fix[v3-1][2])}"
            res_f = [res,res2,res3]
            output_final.append(res_f)
        ...
        class Spinner:
            ...
            def stop(self):
                try:
                    ...
                    print("\n\t"+ bcolors.BG_ERR_TXT+"received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
                    sys.exit(1)
        ...
        if args_namespace.nospinner:
            ...
            if len(unavail_tools_names) == 0:
                ...
                progress("Some tools Not available {}",unavail_tools_names)
               
            progress("Checking Available Security Scanning Tools Phase... Completed.")
            progress("Preliminary Scan Phase Initiated... Loaded {}",str(tool_checks))
            ...
            while(tool < len(tool_names)):
                ...
                text_ = " Deploying "+str(tool+1)+"/"+str(tool_checks)+" | "+ tool_names[tool][arg2]
                progress(text_)
                if tool_names[tool][arg4] == 0:
                    # print(bcolors.WARNING+"\nScanning Tool Unavailable. Skipping Test...\n"+bcolors.ENDC)
                    progress("Scanning Tool Unavailable. Skipping Test..")
                ...
                if runTest == 1:
                        ...
                        # print("\n")
                        text_ = "Scan Completed in "+display_time(int(elapsed))
                        with open("SENTINAL_SCAN.txt","w") as f:
                            for x in output_final:
                                for y in x:
                                    add_text(y)
                        progress(text_)
                        #clear()
                        ...
                        print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit Sentinal Scan\n" + bcolors.ENDC)
                        rs_skipped_checks = rs_skipped_checks + 1

                tool=tool+1

            progress("Preliminary Scan Phase Completed")
            ...
            debuglog = "sentinalscan.debug.%s.%s" % (target, date) 
            vulreport = "sentinalscan.vulnerability.%s.%s" % (target, date)
            # print(bcolors.BG_HEAD_TXT+"[ Report Generation Phase Initiated. ]"+bcolors.ENDC)
            progress("Report Generation Phase Initiated")
            print("Output :",output_final)
            with open("SENTINAL_SCAN.txt","w") as f:
                for x in output_final:
                    for y in x:
                        print(y)
            ...
            if len(rs_vul_list)==0:
                ...
                progress("No vulnerabilities Detected")
            else:
                with open(vulreport, "a") as report:
                    while(rs_vul < len(rs_vul_list)):
                        ...
                        report.write("\n------------------------\n\n")
                        temp_report_name = "/tmp/"+vuln_info[arg1]
                    ...
                    # print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+vulreport+bcolors.ENDC+" is available under the same directory RapidScan resides.")
                        progress("Completed Vulnerability Report")
                report.close()
            ...
            for file_index, file_name in enumerate(tool_names):
                with open(debuglog, "a") as report:
                    try:
                        with open("/tmp/"+file_name[arg1], 'r') as temp_report:
                                data = temp_report.read()
            ...
            text_ = "\tTotal Number of Vulnerability Checks        : "+str(len(tool_names))+"\n\tTotal Number of Vulnerability Checks Skipped: "+str(rs_skipped_checks)+"\n\tTotal Number of Vulnerabilities Detected    : "+str(len(rs_vul_list))+"\n\tTotal Time Elapsed for the Scan             : "+display_time(int(rs_total_elapsed))
            progress_label.configure(text_)
...
logo_label = customtkinter.CTkLabel(sidebar_frame, text="SentinalScan", font=customtkinter.CTkFont(size=20, weight="bold"))
logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))
appearance_mode_label = customtkinter.CTkLabel(sidebar_frame, text="Appearance Mode:", anchor="w")
appearance_mode_label.grid(row=5, column=0, padx=20, pady=(10, 0))
appearance_mode_optionemenu = customtkinter.CTkOptionMenu(sidebar_frame, values=["Light","Dark"],  command=change_appearance_mode_event)

appearance_mode_optionemenu.grid(row=6, column=0, padx=20, pady=(10, 10))
main_frame = customtkinter.CTkFrame(root, width=140, corner_radius=0,fg_color="transparent")
main_frame.grid(row=0, column=1, rowspan=4,columnspan=4 ,sticky="nsew")

# root.logo_label = customtkinter.CTkLabel(root.main_frame, text="Welcome,", font=customtkinter.CTkFont(family="Helvetica",size=30))
# root.logo_label.place(x=50,y=50)
entry = customtkinter.CTkEntry(main_frame, placeholder_text="Target URL",width=500,height=50)
entry.place(x=200,y=80)
scan_button = customtkinter.CTkButton(master=main_frame, text="Start Scan", fg_color="transparent", border_width=2, text_color=("gray10", "#DCE4EE"),command=fun)
scan_button.pack(side="bottom",ipady=6,fill="x",padx=30,pady=10)


scan_label = customtkinter.CTkLabel(main_frame, text="", font=customtkinter.CTkFont(size=20))
scan_label.place(x=200,y=360)



progress_label = customtkinter.CTkLabel(main_frame, text="", font=customtkinter.CTkFont(size=15))
progress_label.place(x=210,y=432)
